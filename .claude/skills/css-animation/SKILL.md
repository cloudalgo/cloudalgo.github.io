...
.element-hidden {
  ...
  transition:
    ...
    visibility 0s 0.5s; /* visibility delays on hide */
}
.element-hidden.visible {
  ...
  transition:
    opacity 0.5s ease-out,
    visibility 0s 0s; /* visibility instant on show */
}
```
...
JSON.stringify({
  ...
  fits: content.scrollHeight <= stage.offsetHeight - content.offsetTop,
});
```
